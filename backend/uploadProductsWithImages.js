...
// Scarica immagine e caricala su Firebase Storage
async function uploadImageFromUrl(url, filename) {
  const res = await fetch(url);
  const buffer = await res.buffer();
  const filePath = path.join("/tmp", filename); // temporaneo
...
async function uploadProducts() {
  ...
  for (let product of products) {
    try {
      ...
      const imageUrl = await uploadImageFromUrl(product.image, imageName);

      const productWithFirebaseImage = {
        ...product,
        image: imageUrl,
      };

      const docRef = await addDoc(productsRef, productWithFirebaseImage);
      console.log(`✅ ${product.title} caricato con ID ${docRef.id}`);
    } catch (e) {
